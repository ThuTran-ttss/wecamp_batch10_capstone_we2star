...
// ─── Map category → icon (giống BudgetOverview dùng lucide) ───────────────────
const CATEGORY_ICON = {
  Transport: Plane,
  Hotel: Hotel,
  Food: Utensils,
...
// ─── Helper: format date thành nhãn "Today", "Tomorrow", hoặc "Thu, Jun 12" ──
function formatDayLabel(dateStr, tripStartDate) {
  ...
  if (diffDays === 0) prefix = `Today, ${date.toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" })}`;
  else if (diffDays === 1) prefix = `Tomorrow, ${date.toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" })}`;
  else prefix = date.toLocaleDateString("en-US", { weekday: "short", month: "short", day: "numeric", year: "numeric" });

  return `${prefix} (Day ${dayNumber})`;
}

// ─── Sub-component: một activity row ─────────────────────────────────────────
function ActivityRow({ activity }) {
  const IconComponent = CATEGORY_ICON[activity.category] || Package;
  const statusStyle = STATUS_STYLE[activity.status] || STATUS_STYLE["Planned"];

  return (
    <div className="flex items-center gap-3 py-3">
      {/* Time */}
      <span className="w-11 shrink-0 text-right text-xs font-semibold text-gray-400">
        {activity.time}
      </span>
...
function UpcomingItinerary({ itineraryData = [], tripStartDate = "" }) {
  ...
  // Chỉ lấy 3 ngày đầu tiên để hiển thị (upcoming)
    ...
    <div className="flex flex-col gap-4 rounded-2xl border border-gray-100 bg-white p-6 shadow-sm">
      ...
      {isEmpty ? (
        <div className="flex flex-col items-center justify-center py-10 text-center">
          <CalendarDays size={36} className="mb-2 text-gray-200" strokeWidth={1.5} />
          ...
          {displayDates.map((date) => (
            <div key={date}>
              ...
              <p className="mb-1 text-sm font-bold text-blue-500">
                {formatDayLabel(date, tripStartDate)}
              </p>
...
//         </div>
//       ) : (
//         /* Scrollable container — ~2 day cards tall */
//         <div
